...
class App
  module Views
	...
	results=db.query("SELECT COUNT(*) AS undoc FROM field_definitions WHERE table_schema='#{schema}' AND table_name='#{table}' AND (status IS NULL OR status NOT LIKE '%#2')")
	...
	results=db.query("SELECT table_schema, table_name, column_name, notes, CONCAT(table_schema, '.', table_name, '.', column_name) AS tkey FROM field_definitions WHERE table_schema='#{schema}' AND table_name='#{table}' AND (status IS NULL OR status NOT LIKE '%#2')")
	results.each do |row|
